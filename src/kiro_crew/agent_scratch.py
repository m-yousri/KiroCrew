...
def _read_owner_pids(marker: Path) -> tuple[int, ...]:
    ...
    has it so a link planted after the lstat is not followed either, checked
    ...
    try:
        ...
        data = os.read(fd, _OWNER_MARKER_MAX_BYTES + 1)
    finally:
        os.close(fd)
...
def adopt_owner(path: Path, pid: int) -> OwnerOutcome:
    ...
    *pid* lives" already holds, and a marker naming only *pid* would make the
    ...
    ``"refused"`` without a write, like a planted link.
    ...
    try:
        with _ADOPT_LOCK:
            try:
                ...
                # regular file): the directory's owner has replaced it, which
                # is the same steering a planted link is. Nothing is written.
                logger.warning(
                    "agent-scratch: refusing to join an unreadable %s in %r",
                    OWNER_FILENAME,
                    path.name,
                )
                return "refused"
...
def shared_scratch_window(path: Path | None) -> Path | None:
    ...
    with _SWEEP_LOCK:
        ...
        try:
            ...
            # Fail-open: the tree is still valid, it just did not get the
            # grace-window refresh -- the same standing it had before.
            logger.debug(
