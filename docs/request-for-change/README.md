...
|---|---|---|
| [rfc-redaction-explain-and-reveal.md](rfc-redaction-explain-and-reveal.md) | `draft` | Rollout step 1 only, shipped with the doc's own PR: widget-frame CSP was already on main (`widgetSrcdoc.ts` `cspFor` — `default-src 'none'`, `img-src data: blob:`); the PR makes deferred remote media UNCONDITIONAL on every `MarkdownRenderer` surface (`img`, `video`, `audio`, `source`; no prop, context, or caller opt-out survives review) and turns server-side previews always off on Slack (`unfurl_*` forced false, endpoint refuses explicit `true`, agent Block Kit media refused), Discord (`SUPPRESS_EMBEDS`) and Telegram (`link_preview_options` on every text-bearing call). Steps 2–6 (placeholder source metadata, lock-tag card, blocked-link chip, phone sheets, percent-decode fix, Settings section) unstarted |
