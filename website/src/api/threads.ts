...
export const threadsApi = {
  ...
  reply: (slot: string, mid: string, text: string): Promise<{ reply: ThreadReply; run_id: string }> =>
    post(`/api/chat/threads/${encodeURIComponent(mid)}/reply`, { slot_key: slot, text }).then(j) as Promise<{ reply: ThreadReply; run_id: string }>,
}
