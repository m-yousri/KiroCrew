...
rules can come from project or user settings, including files already present in a
cloned repository. Crew writes a session-scoped
`.claude/settings.local.json` only when it can prove ownership of that file. The
seed carries a requested `permissions.defaultMode` and deny rules translated from
`disabledTools`, but it neither restricts `settingSources` nor installs a Claude
`PreToolUse` hook, and it never merges into a foreign project settings file.
Therefore an unrelated `permissions.allow` entry inherited from the project or
`~/.claude` can still pre-approve a call before `canUseTool`; an explicitly
requested `bypassPermissions` mode does so by design.
