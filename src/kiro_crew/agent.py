...
def require_foreign_spec_ceiling(
    ...
    project-shadowed name in the form the host DISPATCHES (kiro-cli resolves
    ``--agent`` against ``<cwd>/.kiro/agents`` first, JSON form only, so only
    a parseable project JSON spec keeps the foreign shared spec from being
    consumed — a same-name project Markdown spec or a malformed file makes no
    claim the host honors, and the gate runs); and an ungoverned host
    ...
    if not spec_path.exists():
        return None
    # The stand-aside must match kiro-cli's RESOLUTION, not the checkout's
    # claims: kiro-cli dispatches ``--agent`` against the JSON project form
    # only, so a same-name project ``*.md`` (or a spec that does not parse)
    # does not shadow — the host loads the foreign shared spec anyway, and a
    # name-set stand-aside here would skip the very verification that load
    # needs. ``_project_shadow_of`` narrowed to the dispatchable JSON form is
    # the exact predicate for "the host reads the project copy instead".
    if _project_shadow_of(agent, project_dir, markdown_specs=False, dispatchable_only=True):
        return None
