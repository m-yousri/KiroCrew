...
class _Probe:
    ...
    def window(self) -> dict[str, Any]:
        ...
        stats["window_secs"] = round(max(0.0, now - window_from), 3)
        return stats
...
def _deep_sample(seconds: float, hz: int) -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            ...
            "gateway, and the gateway does not spawn its own tracer."
        ),
...
def _pyspy_output_path() -> "Any":
    ...
    A path under the system temp directory, not a directory created here: nothing
    is executed, so nothing needs cleaning up, and pre-creating a directory for a
    command that may never run is how a directory gets left behind.
    """
    import tempfile
    from pathlib import Path

    return Path(tempfile.gettempdir()) / f"kirocrew-gil-{os.getpid()}.folded"
