...
**Long work, "keep checking", "babysit" or "monitor":** use bounded `monitor_watch` when typed provider facts fully decide supported pull-request review readiness; use `monitor_start` for unsupported targets, advisory evidence, scheduled action, or a required final report. Generic comments and silence-dependent work require `monitor_start` with `gate=false`.

`monitor_watch(kind='github_pull_request', target=<full PR URL>, objective='review_ready')` probes without model turns and wakes only for actionable change. Give it positive runtime, turn, token and provider-error budgets; inspect with `monitor_inspect` and stop with `monitor_stop`, not `autonudge_stop`. It supports dashboard, Slack and Discord; Webex uses `monitor_start`.

`monitor_start(message, interval_secs?, gate?, max_cycles?, max_runtime_secs?, banner?)` keeps this session's context across restarts. Include bounded checks, actions, an exit condition and `autonudge_stop`; revise it with `monitor_update`. A create-only collision preserves the existing automation, so inspect instead of re-arming. Persist long-horizon state with `session_ledger_record` and resume with `session_ledger_read`.
