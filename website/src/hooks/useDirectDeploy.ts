...
export function readRefusal(data: Record<string, unknown>): DeployRefusal | null {
...
export function useDirectDeploy(slug: string, opts?: { onDeployed?: (url: string) => void }) {
  ...
  const start = useCallback(async (profile: string) => {
    ...
    try {
      ...
      setPhase({ kind: 'failed', message: str(data.error) || 'Unexpected response' })
    } catch (e: unknown) {
      setPhase({ kind: 'failed', message: e instanceof Error ? e.message : 'Deploy failed' })
    }
  }, [slug, ttlHours])
  ...
  const confirm = useCallback(async (
    ...
    try {
      ...
      if (preview.profile) body.expected_profile = preview.profile
      ...
      if (str(data.code) === 'stale_preview') {
        setPhase({
          kind: 'failed',
          message: str(data.error) || 'The app changed since it was checked — try again.',
        })
        return
      }
      if (status === 409 && data.blocked && data.reason === 'scan') {
