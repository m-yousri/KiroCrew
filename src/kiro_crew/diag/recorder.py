...
WATCHED_CONFIG_FILES = ("config.json", "config.local.json", ".env")
...
class Recorder:
    ...
    def query(
        ...
        *cursor* resumes a previous answer that hit :data:`QUERY_BYTE_CAP`;
        ...
        if cursor:
            try:
                start_at = max(window_from, float(cursor))
        ...
        for row in self._iter_rows(window_from, window_to):
            total_rows += 1
            if not row.get("kind"):
                self._accumulate(row, accumulators)
            ...
            if cost > budget and (series or events):
                # Cut on a row boundary and hand back where to resume.
                next_cursor = repr(row["ts"])
                continue
            budget -= cost
    ...
    def _resolve_window(
        ...
        if around is not None:
            span = float(radius if radius is not None else DEFAULT_QUERY_RADIUS_SECS)
            return float(around) - span, float(around) + span
        now = self._clock()
        end = float(until) if until is not None else now
        begin = float(since) if since is not None else end - DEFAULT_QUERY_RADIUS_SECS
        if begin > end:
            begin, end = end, begin
        return begin, end
    ...
    @staticmethod
    def _accumulate(row: dict[str, Any], out: dict[str, list[float]]) -> None:
        """Collect numeric leaves as dotted paths, one level into nested blocks."""
