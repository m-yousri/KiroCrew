...
def test_a_rewritten_config_file_emits_an_event_without_reading_its_bytes(
    ...
    ``.env`` is one of the watched files and it holds credentials, so the secret
    below is the assertion: if any code path read the file to detect the change,
    its contents would reach a snapshot row.
    """
    recorder = _make(tmp_path)
    home = tmp_path / "home"
    ...
    assert [r["payload"]["file"] for r in rewrites] == [".env"]
