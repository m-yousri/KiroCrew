...
vi.mock('../../api/client', () => ({
  api: {
    ...
    memberBriefing: vi.fn(() => Promise.resolve({ slug: '', member: '', supported: true, text: '', updated_ts: null, path: '' })),
...
beforeEach(() => {
  ...
  vi.mocked(api.memberBriefing).mockImplementation(() =>
    Promise.resolve({ slug: '', member: '', supported: true, text: '', updated_ts: null, path: '' }),
  )
...
describe('MembersPage Notes tab (the crewmate\'s own notes)', () => {
  const briefing = (over: Record<string, unknown> = {}) => ({
    ...
    path: '/home/u/.kiro/crew/members/oncall/briefing.md',
    ...over,
  })
  ...
  it('renders the briefing as markdown, with when it was last written and an Edit that opens the file in the panel', async () => {
    ...
    const footer = within(notes).getByTestId('member-notes-footer')
    expect(footer).toHaveTextContent(/^Updated /)
    const edit = within(footer).getByTestId('member-notes-edit')
    expect(edit).toBeEnabled()
    expect(edit).toHaveTextContent('Edit notes')
    // Edit is a file open in THIS panel (the file viewer tab), not a navigation.
    const opened = vi.fn()
    window.addEventListener('kirocrew-file-open', opened as EventListener, { once: true })
    fireEvent.click(edit)
    await waitFor(() => expect(opened).toHaveBeenCalled())
    expect((opened.mock.calls[0][0] as CustomEvent).detail.path).toBe('/home/u/.kiro/crew/members/oncall/briefing.md')
    expect(currentUrl()).toContain('/members')
  })

  it('no notes yet is an EMPTY state naming the crewmate, with Edit disabled — never an error', async () => {
    vi.mocked(api.memberBriefing).mockResolvedValue(briefing({ text: '', updated_ts: null, path: '' }))
    const notes = await openNotes()
    ...
    expect(within(notes).getByTestId('member-notes-edit')).toBeDisabled()
    expect(within(notes).getByTestId('member-notes-footer')).not.toHaveTextContent(/Updated/)
  })
  ...
  it('two crewmates sharing the slug: the read is refused and the panel says so in plain words, with no Edit', async () => {
...
describe('MembersPage Work log — session record', () => {
  it('embeds the thread\'s Crew Log once the slot is confirmed, under its own heading', async () => {
    ...
    expect(record).toHaveTextContent('Session record')
