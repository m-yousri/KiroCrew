...
- `handlers/members.py`, continued — the one-time crewmate opt-in: `GET /api/members/optin` (`{done, crewmates, candidates}` — the persisted `dashboard.crewmate_optin_done` gate, the registered crew count, and the custom agents no crew is built from with their `agent_usage()` chat counts; app tokens 404, scan failure 503 `optin_candidates_unavailable`) and `POST /api/members/optin/done` (owner-only, idempotent delta write of the gate under `run_config_write`). Registered before the `{slug}` routes so a crew slugged `optin` cannot shadow them. Contract: [crew-mode](crew-mode.md#one-time-opt-in-for-existing-custom-agents).
