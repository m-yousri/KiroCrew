...
- `handlers/teams.py` — crewmate teams, the Crewmates page's roster grouping and team view. A team is a name plus an ORDERED list of exact crew names (`kiro_crew.crew_teams.Team` — `crew_teams`, because `kiro_crew.teams` is the Microsoft Teams channel package and a same-named module would be shadowed by it), nothing else: no memory, no chat, no prompt of its own; everything the team view shows is derived from its members' existing threads, activity records and live presence. Storage is one document, `teams.json` in its own data-home directory `crew-teams/` (`{"version": 1, "teams": [{id, name, members}]}` — `TEAMS_SCHEMA_VERSION`, written on every save; a document from a NEWER build is refused whole). The directory is deliberately NOT under `trust/`: `trust` is a declared sandbox READ-WRITE exception (in-sandbox code appends to the audit log there), so a record under it stays writable by a sandboxed command that builds the path at runtime — the `crew-panels` lesson. `crew-teams` is instead a whole-directory leaf on `sandbox._CREW_HIDDEN_LEAVES` (masked from every sandboxed process) and `sandbox._CREW_PRECREATE_HIDDEN_DIR_LEAVES` (materialised empty at 0700 before every spawn, because the store is created on the first team and `mount(2)` cannot mask an absent name) and on `security._CREW_SECRET_LEAVES` (agent file tools refuse read and write); only the gateway, on a human dashboard action, opens it, so a crewmate's own tools cannot re-team itself or its siblings (`test_store_directory_is_masked_and_fenced`). Every mutation runs read → mutate → rewrite under TWO locks — a process-wide `threading.Lock` (`_WRITE_LOCK`) and, inside it, an advisory file lock on `crew-teams/.lock` (`platform_compat.file_lock`, `crew_teams._document_lock`): the gateway's writers (owner routes, crew delete, package prune) run off-loop in worker threads, `kirocrew agents delete` writes from ANOTHER process, and only the file lock reaches it; a contended acquire past the default ceiling fails closed as `OSError` → 500 `teams_write_failed` rather than writing unserialized (`test_writers_hold_a_cross_process_file_lock`) — atomically with fsync (`crew_teams.write_teams`), and the read (`crew_teams.read_teams`) treats an ABSENT file as no teams but an existing-yet-unparseable one as `TeamsUnreadable`, which every route answers as 500 `teams_unreadable` rather than an empty list — reading it empty would let the next write erase every team the user made. Two invariants the store enforces so no UI has to: a crewmate is on AT MOST ONE team (`create_team` / `update_team` detach the listed names from every other team in the same write; a foreign document listing a name twice reads as the FIRST team, `_parse_teams`), and names are validated against the registered crews at write time (`unknown_member`), never against slugs — slugification is lossy and the roster is keyed by exact name. Ids are 12 random hex chars (`_new_team_id`), path- and query-safe; the routes refuse any other spelling with 400 `invalid_team_id` before touching the file. Names are stripped, capped at `TEAM_NAME_MAX_CHARS` (refused, never truncated), and refuse lone surrogates plus Unicode category `Cc` / `Zl` / `Zp` — control characters and line separators (a roster header is not a place for a terminal-control payload); format characters (a ZWJ emoji sequence) and the no-break space are legitimate in a pasted name and pass. `drop_member` is the crew-DELETE path's hook (`api_kirocrew_agent_delete`, best-effort off-loop): a team must not keep counting a crewmate with no roster row; a stale name is otherwise tolerated by every reader, which is why BOTH an unreadable document and a failed rewrite (`OSError`) are logged and swallowed there — the crew's config write has already committed, and neither may turn a completed delete into a 500. The document is bounded on WRITE as well as read: `TEAMS_FILE_MAX_BYTES` is sized above `max_document_bytes()` (the largest document every cap admits, pinned by `test_a_valid_write_can_never_outgrow_the_read_cap`), and a save that would exceed it is refused with 400 `teams_too_large` before anything lands, so a valid sequence of writes can never leave the store `teams_unreadable`. Two removal paths never reach `drop_member` — `kirocrew agents delete` runs in another process, and the package sync prunes agents whose package is gone — so both call it best-effort — the CLI under the file lock, the sync prune ONLY for the names its locked mutate actually deleted (a snapshot candidate edited concurrently survives and keeps its team) — AND `GET /api/teams` reconciles the document against the live registry on every read (`crew_teams.prune_unknown`): an API answer never names a crewmate the registry lacks, whatever path removed it, while the document itself is left as written and re-validated by the next membership write. Routes: `GET /api/teams` (`{teams}`), `POST /api/teams {name, members}` → 201 `{team}`, `PUT /api/teams/{id} {name?, members?}` → `{team}` (omitted fields unchanged; both omitted is 400 `nothing_to_update`), `DELETE /api/teams/{id}` → `{ok}`; unknown id is 404 `team_not_found`, malformed bodies are coded 400s (`invalid_json`, `invalid_team_name`, `team_name_too_long`, `invalid_members`, `unknown_member`, `too_many_teams`, `too_many_members`, `teams_too_large`), a failed persist is 500 `teams_write_failed`. The create and update routes load the config snapshot and write the team INSIDE the crew registry's config lock (`agents._get_config_lock`, resolved late), the same lock the crew-delete route holds from removing the crew through `drop_member`, so a delete cannot interleave between "the crew exists" and "the team is written" and leave a team naming a gone crewmate. Same posture as the members routes: app tokens are denied with 404 on every route (`_deny_app_caller`), and the three writes are OWNER actions (`require_owner_dashboard_request`, gated BEFORE body validation so a non-owner's answer stays a uniform 401/403 that leaks nothing about what would have validated).
