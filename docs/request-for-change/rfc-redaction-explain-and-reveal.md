...
Keep a suspicious URL visible as a **Blocked link** chip, but do not make it
clickable. Show the site name and path immediately, and hide the query until
the user selects *Inspect*. The details card explains why the link was blocked
and can show the full URL; *Copy URL* copies it without opening it. *Open
once* requires confirmation, with Cancel selected by default, and the
confirmation names both the site and the query that will be sent. *Allow long
...
rule applies before offering *Undo*. Users can always cancel *Open once*. On
...
3. Replace deleted URLs with a Blocked link chip and an Inspect button. Let
   only the session owner use *Open once*, with Cancel selected by default, or
   create a per-workspace Allow rule with confirmation and Undo. Only the
   dashboard can write this list; Settings → Security → Redaction shows each
   entry and lets the user revoke it.
...
Each step is independent, so it can be released or rolled back without
requiring the later steps.

### Step 1 status

Shipped with this document's PR, verified against `3df403cdb`:
...
- **Deferred remote media** lands in this PR and is UNCONDITIONAL on every
  `MarkdownRenderer` surface: agent-authored markdown is the common case, so
  the gate is inherited by construction, and after review rounds removed
  first the caller prop and then the context itself, no renderer can forget
...
- **Slack unfurls are always off** in this PR: `RealSlackClient.post_message`
  ...
  dropping it; `false`/absent stay accepted. Agent-supplied Block Kit is
  refused (HTTP 400, code `blocks_remote_media_disabled`) when it carries
  `image`/`video` blocks or `image_url`/`thumbnail_url`/`video_url` fields
  anywhere in the tree, because Slack fetches that media server-side
  regardless of the unfurl flags.
