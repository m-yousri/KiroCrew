...
#: selection below is keyed on it; the point's own module (``points/nudge_wake``)
...
def _oracle(lane: str, provider: Any) -> Any:
    ...
    *extra* is written onto whichever row this call produces -- the round of a
    ...
    try:
        answers = await asyncio.wait_for(
            _oracle(lane, provider).ask(state, questions), timeout=budget
        )
